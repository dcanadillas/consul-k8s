	...
	for _, c := range cases {
		t.Run(c.TestName, func(t *testing.T) {
			...
			cmdArgs := append([]string{
				...
				"-addresses", strings.Split(testAgent.HTTPAddr, ":")[0],
				"-http-port", strings.Split(testAgent.HTTPAddr, ":")[1],
				"-grpc-port", strings.Split(testAgent.GRPCAddr, ":")[1],
				"-resource-prefix", resourcePrefix,
			}, c.TokenFlags...)
...
func TestRun_HTTPS(t *testing.T) {
	...
	responseCode := cmd.Run([]string{
		...
		"-addresses=" + strings.Split(srv.HTTPSAddr, ":")[0],
		"-http-port=" + strings.Split(srv.HTTPSAddr, ":")[1],
		"-grpc-port=" + strings.Split(srv.GRPCAddr, ":")[1],
	})
	require.Equal(t, 0, responseCode, ui.ErrorWriter.String())
...
func replicatedSetup(t *testing.T, bootToken string) (*fake.Clientset, *api.Client, string, string, string) {
	primarySvr := test.TestServerWithConnMgrWatcher(t, func(c *testutil.TestServerConfig) {
		c.ACL.Enabled = true
		if bootToken != "" {
	...
	secondarySvr := test.TestServerWithConnMgrWatcher(t, func(c *testutil.TestServerConfig) {
		c.Datacenter = "dc2"
		c.ACL.Enabled = true
